...
c0 <- d %>%
  ...
  mutate(region = countrycode(sourcevar = club_alpha3, origin = "iso3c", 
                              destination = "continent")) %>%
  replace_na(list(region = "Europe")) %>%
  arrange(region, club_country) %>%
  mutate(
    club_country = fct_inorder(club_country),
    club_country = fct_relevel(club_country, "No Club")) %>%
...
r <- n0 %>%
  ...
  mutate(gap = ifelse(kit_shirt == "transparent", 2.5, 1.5),
         label = case_when(
           ...
           lab == "Republic of Ireland" ~ "Ireland",
           lab == "Georgia (country)" ~ "Georgia",
           lab == "The Gambia" ~ "Gambia",
           lab == "Democratic Republic of the Congo" ~ "DR Congo",
           lab == "Burkina Faso" ~ "Burkina\nFaso",
           lab == "Guinea-Bissau" ~ "Guinea-\nBissau",
...
# plot into pdf
pdf(file = "./plot/afcon_2021_chord.pdf", useDingbats = FALSE)
...
circos.trackPlotRegion(
  ...
  panel.fun = function(x, y) {
    ...
    circos.text(x = xx, y = ifelse(flag_disp, 0.38, 0), 
...
dev.off()
file.show("./plot/afcon_2021_chord.pdf")

library(magick)
p <- image_read_pdf("./plot/afcon_2021_chord.pdf")
# pp <- image_read_pdf("./plot/euro_2020.pdf")
# pp

logo <- paste0("./logo/", 2021, ".png") %>%
  image_read(density = 300) %>%
  image_resize(geometry = "350x350") %>%
  image_extent(geometry = "300x350", gravity = "north-west") %>%
  image_border(color = "grey40", "5x5")
logo
...
https://en.wikipedia.org/wiki/2021_Africa_Cup_of_Nations"
  ...
  image_write(path = "./plot/afcon_2021.png")

file.show("./plot/afcon_2021.png")
